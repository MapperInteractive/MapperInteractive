define(function (require) {
  ...
  return app.view({
    ...
    initialize: function () {
      ...
      this.data = app.states();

      this.listenTo(this.states, 'change:graph', () => this.onGraphChange());
      this.listenTo(this.states, 'change:nodes', () => this.onNodesChange());
      this.listenTo(this.states, 'change:items', () => this.onItemsChange());
      this.listenTo(this.data, 'change', () => this.render());
    },

    onGraphChange: function () {
      this.states.get('graph').on('change:selection', (nodes) => {
        this.states.set('nodes', nodes);
      });
    },

    onNodesChange: function () {
      let nodes = this.states.get('nodes');
    ...
    onItemsChange: function () {
      this.items = {};
      this.states.get('items').forEach((item) => {
