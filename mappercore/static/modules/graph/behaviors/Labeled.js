    ...
    _createClass(Labeled, [{
      ...
      value: function didMount() {
        ...
        this.events.listenTo(this.graph, 'didLayout', function () {
          _this2.graph.svg.selectAll('.viewer-graph__label').data(_this2.graph.nodes.data()).attr('dx', function (d) {
            return d.x;
          }).attr('dy', function (d) {
            return d.y + 1;
      ...
      value: function _graphDidRender() {
        this.labels = this.graph.svg.append("g").selectAll("text").data(this.graph.nodes.data()).enter().append("text").attr("dx", 12).attr("dy", ".35em").text(function (d) {
          return d.id;
