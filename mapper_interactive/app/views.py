...
def process_text_data():
    ...
    mat = [
        n.split(",") for n in text_data
    ]  # csv: if an element is empty, it will be "".
...
def load_mapper_data():
    ...
    mapper_graph_new = _parse_result(
        mapper_graph, lens_dict={}, data_array=[], if_cli=True
    )
    connected_components = compute_cc(mapper_graph_new)
    return jsonify(
        mapper=mapper_graph_new,
        connected_components=connected_components,
        categorical_cols=mapper_graph["categorical_cols"],
    )
...
def get_graph():
    ...
    if norm_type == "none":
        ...
        data = sklearn.preprocessing.normalize(
            data, norm=norm_type, axis=0, copy=False, return_norm=False
        )
    ...
    if len(categorical_cols) > 0:
        for node in mapper_result["nodes"]:
            ...
            for col in categorical_cols:
                node["categorical_cols_summary"][col] = (
                    data_categorical_i[col].value_counts().to_dict()
                )
...
def linear_regression():
    ...
    return jsonify(
        ...
        pvalues=list(result.pvalues),
        conf_int=conf_int_new,
        stderr=list(result.bse),
    )
...
def pca():
    ...
    # clustering
    if len(selected_nodes) > 0:
        data_new["kmeans_cluster"] = (
            KMeans(n_clusters=min(len(selected_nodes), 6), random_state=0)
            .fit(data_new)
            .labels_
        )
    else:
        ...
        data_new["kmeans_cluster"] = (
            KMeans(n_clusters=6, random_state=0).fit(data_new).labels_
        )
...
def run_mapper(
    ...
    km_result, lens_dict = _call_kmapper(
        data_array,
        col_names,
        interval,
        overlap,
    ...
    if clustering_alg == "DBSCAN":
        graph = mapper.map_parallel(
            ...
            clusterer=cluster.DBSCAN(
                eps=float(clustering_alg_params["eps"]),
                min_samples=int(clustering_alg_params["min_samples"]),
            ),
        ...
        graph = mapper.map_parallel(
            ...
            clusterer=cluster.AgglomerativeClustering(
                n_clusters=None,
                linkage=clustering_alg_params["linkage"],
                distance_threshold=float(clustering_alg_params["dist"]),
            ),
            cover=Cover(n_cubes=interval, perc_overlap=overlap),
        )
        ...
        graph = mapper.map_parallel(
            ...
            clusterer=cluster.MeanShift(
                bandwidth=float(clustering_alg_params["bandwidth"])
            ),
            cover=Cover(n_cubes=interval, perc_overlap=overlap),
        )
...
def compute_lens(f, data, mapper, filter_parameters=None):
    ...
    if f in ["sum", "mean", "median", "max", "min", "std", "l2norm"]:
        ...
        kde = KernelDensity(kernel=density_kernel, bandwidth=density_bandwidth).fit(
            data_array
        )
        ...
        lens = np.array(
            [(np.sum(pdist**p, axis=1) / len(data_array)) ** (1 / p)]
        ).reshape(-1, 1)
    elif f == "PC1":
...
def call_module_function(data, cols, module_info):
    ...
    if module_info["module-type"] == "unsupervised_learning":
        ...
        data_new["kmeans_cluster"] = (
            KMeans(n_clusters=4, random_state=0).fit(data_new).labels_
        )
        data_new = data_new.to_json(orient="records")
