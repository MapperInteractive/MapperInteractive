...
from .graph import EnhancedGraph, Graph
...
def graph_to_networkx(g, enhanced=True):
    if not nx_available:
        raise ModuleNotFoundError(
            "Networkx could not be found. This functionality is unavailable."
        )
    nx_g = nx.Graph()
    ...
    nx_g.add_edges_from(
        [
            (
                node2compactrep(n[0], enhanced=enhanced),
                node2compactrep(n[1], enhanced=enhanced),
            )
            for n in g.edges
        ]
    )
...
def to_mapper_interactive_json(graphs, num_intervals, file_dir):
    ...
    md = {
        "data": "./sample_datasets/3d-horse.csv",
        "overlaps": "20:20:5",
        "filter": "y",
        "normalization": None,
        "dbscan_eps": 0.3,
