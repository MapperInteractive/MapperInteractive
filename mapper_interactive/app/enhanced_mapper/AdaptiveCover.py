...
def bic_centroid(X, c, assignments, BIC=True):
    ...
    for i in range(k):
        ...
        log_term += cluster_members.shape[0] * (log(cluster_members.shape[0]) - log(R))
...
def get_new_centroids(X, centroid):
    ...
    random_direction = random_direction / np.sqrt(
...
def xmeans(
    X, centroids, iterations=1, region_iterations=1, max_k=-1, BIC=True, verbose=False
):
    ...
    for i in range(iterations):
        ...
        for c in range(len(default_bic)):
            ...
            if BIC and var != 0:
                default_bic[c] = xmeans_log_likelyhood(r, X.shape[1], var, 1, r) - (
                    p / 2
                ) * log(
                    r
                )  # bic
                ...
                # aic
                default_bic[c] = (
                    2 * xmeans_log_likelyhood(r, X.shape[1], var, 1, r) - (p) * 2
                )
            else:
        ...
        for c in range(len(default_bic)):
            ...
            for region_attempt in range(region_iterations):
                ...
                if default_bic[c] < new_bic and (
                    best_region_run is None or best_region_run < new_bic
                ):
        ...
        new_centroids = []
        for i, to_split, centroid, new_centers in zip(
            range(len(split_region)), split_region, centroids, split_centroids
        ):
        ...
        if method is None or method == "BFS":
            for i in range(cover.num_intervals):
                ...
                (
                    ...
                    interval_membership,
                    interval_members,
                ) = g.to_hard_clustering_set(X, intervals=[i])
                ...
                (
                    ...
                    interval_membership,
                    interval_members,
                ) = g_split.to_hard_clustering_set(X, intervals=[i, i + 1])
                if len(interval_centers) == 0:
            ...
            element_lengths = [
                cover[i][1] - cover[i][0] for i in range(cover.num_intervals)
            ]
            found_valid = False
            while not found_valid and len(all_elements_idx) != 0:
                ...
                (
                    ...
                    interval_membership,
                    interval_members,
                ) = g.to_hard_clustering_set(X, intervals=[current_element])
                if len(interval_centers) == 0:
                ...
                (
                    ...
                    interval_membership,
                    interval_members,
                ) = g_split.to_hard_clustering_set(
...
def construct_cover_from_xmeans(
    X,
    lens,
    initial_interval,
    starting_overlap,
